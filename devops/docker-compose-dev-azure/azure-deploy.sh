...
resourcename="test"  # lower case, only
...
# git clone https://github.com/apilogicserver/test.git
# cd test
...
az group create --name test_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group test_rg --sku S1 --is-linux
az container create --resource-group test_rg --name test --image apilogicserver/test:latest --dns-name-label test --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//test.westus.azurecontainer.io:5656'
